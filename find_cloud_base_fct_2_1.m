...
%
%
%       difference to version 2.0 :     version 2.1 does not give clouds, outside the height_rof_cloudsearch_L




function [cloudbase_L,time_rof_found_cloudbases_L] = find_cloudbase_fct_2_1 (signal_L,height_rof_cloudsearch_L,...
                                                                                    depol_out_cal_rof_height_L,...
...
for t_L=1:size(signal_L,2)                                                  % go through all time bins.  size(signal,2) gives the no of elements along 2nd dimension which is the time bins
    ...
    if max_signal_index_L(t_L)>100                                 % if altitude of max was large enough
        ...
        zmin_L=find( signal_normalized_to_max_L(height_rof_cloudsearch_L,t_L)<0.06,1,'last' )  +  height_rof_cloudsearch_L(1);            %find the last element within one time all heights which is smaller than 0.06 . Add the lower boundary of cloudsearch to have the real height-bin. units are all in bins.
        zmin2_L=find( signal_normalized_to_max_L(height_rof_cloudsearch_L,t_L)>0.09,1,'first' )  +  height_rof_cloudsearch_L(1) - 1;       %find the first element within one time all heights which is larger than 0.09 . Add the lower boundary of cloudsearch to have the real height-bin. substract one bin. units are all in bins.
        %Threshold 2
        if isfinite(zmin_L)                                                     % continue only if first test found some zmin
            if zmin_L > zmin2_L                                                 % zmin>zmin2 -> signal drops after cloud again significantly  -->  Check for multilayer cloud
                ...
                zmin_L = find(signal_normalized_to_max_in_CLOUD_L(100:max_in_cloud_index(t_L),t_L)<0.06,1,'last')+100;  % find max in cloud
                
                % Threshold 3
                if isempty(zmin_L)                                              % If threshold was not reached continue
                    zmin_L = find(signal_normalized_to_max_in_CLOUD_L(100:max_in_cloud_index(t_L),t_L)<0.1,1,'last')+100-4;                 % Set threshold a bit "easier" to reach
                    % Threshold 4
                    if isempty(zmin_L)                                         % If "easier" threshold was NOT reached
                        cloudbase_L(t_L)=NaN;                                                                                            	% No cb found
                        zmin_L = find(signal_normalized_to_max_in_CLOUD_L(100:max_in_cloud_index(t_L),t_L)<0.25,1,'last')+100-15;           % Set zmin a even more  "easier" to reach
                    else
