%% 'Global' Parameters for dushanbe
fprintf('Load Initial Parameters for DUSHANBE CASE !!     ----   by    ------   global_parameters_Dushanbe_case.m  \n\n');
...
Rt_in                       = 1.05;                                          % For Calibration needed inner FOV (but through C also effects outer FOV calibration)
Rc_in                       = 1666;                                           % For Calibration needed inner FOV (but through C also effects outer FOV calibration)
Rt_out                      = 1;
Rc_out                      = 250;                                         
% epsilon_out                 = 1.004;

%Fix C_out on line 335
...
% outer FOV
depol_out_cal_rof_height        = 100:200;%%266:346;                                       %if simple depol_const_out_fct is used 80:180;  60:180??             % This is the range which will be averaged 60:300= 450m-2250m   ( also confines range if depol_const_out_fct is used )
...
height_rof_cloudsearch          = 134:666;                                      % 1005-4995 m  heightrange (bins) for cloud search
...
k_gamma                         = 0.75;%  0.75;  %                                      % 0.75 (for clouds formed around continental airmasses), 0.8 (marine environments). Miles et. al (2000)
...
% time_rof_profile_in_bins    = 400:720;      % currently  set dynamically inside the loop as full time size                  % Time-range of the profile (unit:bins) , One bin is 30sec
height_rof_profile_in_bins  = 1:933;                                        % Height-range of the profile (unit:bins), one bin is 7,5m  . 251 sind die bins die wegen des pretriggers rausgerechnet werden m??ssen. 933=6km
disp('Parameters set.');
...
elseif (ispc)
    ...
    result_path='C:\Robert\Master\Software\ND-Filter\results\dushanbe\';
    data_path='C:\Robert\Master\Daten\Tadjikistan\';
...
InputFiles_lidar=textread('NC_files_dushanbe_some_new.txt','%s','delimiter','\n'); 
...
fprintf('Initial Parameters loaded for DUSHANBE CASE !!     ----   by    ------   global_parameters_Dushanbe_case.m  \n\n');
