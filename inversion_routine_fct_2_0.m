...
function [r_eff_L, alpha_L, lwc_L, cdnc_L, depolratio_allowed_range_L ] = inversion_routine_fct_2_0(depol_in_mean_above_CB_L,depol_ratio_mean_above_CB_L,...
                                                                    cloudbase_inM_L,...
...
R_ref_vs_depolratio_coefficient_matrix  = load('polly_fit_values_R_int.txt');                                                           % Matrix of coefficents of the fit r 70 = r_0 + r_2 · (δ r ± ∆δ r )^2 + r_3 · (δ r ± ∆δ r )^3.  For every simulated_CB_height one coloum of coefficients.
...
for t=1:length(cloudbase_inM_L)                                                                                         % Loop through all measured Cloudbases
    ...
    elseif CB_mean > simulated_cloudbase_heights_L(end)                                                                 % CB is above all simulated_cloudbase_heights_L
        ...
        top_index = length(simulated_cloudbase_heights_L);                                                              % simulated_cloudbase_heights_L(top) is the simulated_cloudbase above the measured CB_mean
    else
    ...
    pd_vector = 1:depth_into_cloud_max;                                                                                 % Creates an array [1, 2 , ... penetrationdepth(in bins)]   pd stands for penetrationdepth
