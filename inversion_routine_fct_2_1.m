...
%
%
%   Difference to version 2.0:
%           Now clouds outside the simulated range are not processed.
...
function [r_eff_L, alpha_L, lwc_L, cdnc_L, depolratio_allowed_range_L ] = inversion_routine_fct_2_1(depol_in_mean_above_CB_L,depol_ratio_mean_above_CB_L,...
                                                                    cloudbase_inM_L,...
                                                                    depth_into_cloud_max,...
                                                                    simulated_cloudbase_heights_L,...
                                                                    k_L)
...
r_ref_input                             = load('polly_input_R_int.txt');                                                                % 1-D Array of all R_ref values which where used in simulation      eg: [2.2551    2.3551    2.4551 ... 13.9550   14.0550]
alpha_ref_input                         = load('polly_input_alpha_int.txt');                                                            % 1-D Array of all alpha_ref values which where used in simulation  eg: [0.0050    0.0051    0.0052   ... 0.0272    0.0273]
R_ref_vs_depolratio_coefficient_matrix  = load('polly_fit_values_R_int.txt');                                                           % Matrix of coefficents of the fit r 70 = r_0 + r_2 ?? (?? r ?? ????? r )^2 + r_3 ?? (?? r ?? ????? r )^3.  For every simulated_CB_height one coloum of coefficients.
for i=1:length(simulated_cloudbase_heights_L)                                                                                       % we load it here all to ommit repetive loading during the cloudbase loop
    depol_simulated_matrix_alpha_r(i,:,:)= load(strcat('polly_alpha_matrix_int_',num2str(simulated_cloudbase_heights_L(i)),'.txt'));    % dim1: index of simulation_height % polly_alpha_matrix is a matrix rownumber:alpha_ref_index, coloumnumber:r_ref_index, values:depol_in
end
...
for t=1:length(cloudbase_inM_L)                                                                                         % Loop through all measured Cloudbases
    ...
    if CB_mean < simulated_cloudbase_heights_L(1)                                                                       % CB is below all simulated_cloudbase_heights_L
        top_index=NaN;
        fprintf('inversion_routine_fct says: %d-th cloudbase was only at %f meters, which is lower than the lowest simulated CB (%f) . Point is not processed \n', ...
                    t,CB_mean,simulated_cloudbase_heights_L(1));
                                                                                                         % simulated_cloudbase_heights_L(top) is the simulated_cloudbase above the measured CB_mean
     elseif CB_mean > simulated_cloudbase_heights_L(end)                                                                 % CB is above all simulated_cloudbase_heights_L
        top_index=NaN;
         fprintf('inversion_routine_fct says: %d-th cloudbase was at %f meters, which is above than the largest simulated CB (%f) Point is not processed \n', ...
                    t,CB_mean,simulated_cloudbase_heights_L(end));
                                                                        % simulated_cloudbase_heights_L(top) is the simulated_cloudbase above the measured CB_mean
    else
    ...
    % Write all NaNs if CB outside range
    if isnan(top_index)
        r_eff_L(:,t)=NaN; 
        alpha_L(:,t)=NaN; 
        lwc_L=r_eff_L(:,t);
        cdnc_L=r_eff_L(:,t); 
        depolratio_allowed_range_L(:,t) = [NaN,NaN];  
        continue;
    end
    ...
    % Note, that:  r_eff_L(max_penetration_depth,:) = R_ref_CB_one_value(:)
   
%    if t==32
%         disp('Set brake point here to analyse 00:40:30 UTC')
%     end
...
%disp('good point for debugging break-point');
