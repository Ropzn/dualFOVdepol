...
function plot_color_3dSignal_plus_2dCloudbase_PART_fct_arielle_case(d3_signal_L,d2_signal_L,...
                                                    num_time_L,height_L,...
...
%Zeit verschiebung
time1_L=7.379260034722222e+05;
time2_L=7.379260036752501e+05;
t_diff_L=time2_L-time1_L;
t_diff_L=0;                                                
                                                
txt= [InputFile,'--Cloudbase and Profile--AUSSCHNITT'];
...
ylimes=[3880 4080];      % in m
% To define color axis use: caxis([0 4]*10^8);


date_tick_range = num_time_L(677:2:end);   % 20-> 10min,    60->1/h std

date_minor_tick_range = num_time_L(37:1:end);


d3_signal_L = squeeze(d3_signal_L);

% shorten height to signal, and convert to km
height_L = height_L(1:size(d3_signal_L,1));

% Cloudbase in km
CB_in_m_L = d2_signal_L * height_L(1);
CB_low_res_in_m = cb_lowres_in_m_L ;
...
% 2D Plot low res
plot(num_time_lowres_L+t_diff_L,CB_low_res_in_m,'o',...
                'LineWidth',2,...
                'MarkerSize',8,...
...
%% 2D Plot
p1=plot(num_time_L+t_diff_L,CB_in_m_L,'LineWidth',2,'Color','blue');             %Cloudbase
for i=1:10
p1=plot(num_time_L+t_diff_L,CB_in_m_L +(7.5*i),'LineWidth',2,'Color','black');  %The 10 bins above CB
hold on
end
%%
