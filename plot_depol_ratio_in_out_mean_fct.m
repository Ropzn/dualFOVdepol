...
function plot_depol_ratio_in_out_mean_fct(s_in,height_L,sig_names,time_rof_profile_in_bins_L,save_all_plots_L,InputFile,result_path_L)

txt= ['Depol RATIO in and out: Mean(Time [' ,num2str(min(time_rof_profile_in_bins_L)), '..' ,num2str(max(time_rof_profile_in_bins_L)), '] of ',sig_names,'--',InputFile ];
txt2= [InputFile ,'--Depol Ratio in and out'];
...
xlimes=[0.5 1.5];        %  Plot Depolrange
...
plot(s_in,height_L,'DisplayName','Ratio');
hold on;
set(gca,'FontSize',12,'YDir','normal');
title(txt,'Interpreter', 'none');
ylabel('Height [km]');
xlabel('Ratio of Depol Time-Means');
