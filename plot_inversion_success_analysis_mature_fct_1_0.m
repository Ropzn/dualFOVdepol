...
function plot_inversion_success_analysis_mature_fct_1_0(cloudbase_inM_lowres_L, time_rof_clouds_lowres_L, ...
...
ylimes_1=[min(cloudbase_inM_lowres_L) max(cloudbase_inM_lowres_L)];

% xlimes=[num_time_L(1) num_time_L(end)];
xlimes=[xstart xend];

num_time_rof_clouds_res=num_time_L(time_rof_clouds_lowres_L);
...
p1=plot(num_time_rof_clouds_res,cloudbase_inM_lowres_L/1000,'o','LineWidth',2,'DisplayName','cloudbase');
...
hold on
plot(num_time_rof_clouds_HIGH_res,cloud_max_Mpcs_L(1,:),'-','DisplayName','PMax^{in}_{tot}','LineWidth',2);
hold on
plot(num_time_rof_clouds_HIGH_res,cloud_max_Mpcs_L(2,:),':','DisplayName','PMax^{in}_{cross}','LineWidth',2);
plot(num_time_rof_clouds_HIGH_res,cloud_max_Mpcs_L(3,:),':','DisplayName','PMax^{out}_{tot}','LineWidth',2);
p2=plot(num_time_rof_clouds_HIGH_res,cloud_max_Mpcs_L(4,:),':','DisplayName','PMax^{out}_{cross}','LineWidth',2);
hold off
set(gca,'fontweight','bold','fontsize',16,'YDir','normal');
% legend( 'Location', 'eastoutside');
...
plot(num_time_rof_clouds_res,depol_ratio_mean_above_CB_L,'o','LineWidth',2,'color',[0.4660, 0.6740, 0.1880],'DisplayName','\delta_{r}');
